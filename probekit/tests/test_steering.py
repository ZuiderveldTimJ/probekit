...
def test_build_steering_vector_linear_probe(mock_sae):
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA not available")
def test_build_steering_vector_legacy_dict(mock_sae):
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA not available")
def test_build_steering_vectors_batched(mock_sae):
    probekit = [
